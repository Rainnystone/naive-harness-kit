...
**怎么挑帮手。** NHK 使用三个 Codex 档位，权限按工作角色确定。常规实施、局部设计、集成、排错和独立调查默认使用 medium。较轻档位可以完整实现并测试明确、低风险的小功能，也可以处理机械工作：行为与接口明确、有既定做法、验证可靠、影响局部可控；计划不必给出完整实现代码。涉及安全、数据完整性或隐蔽的跨任务影响时，即使代码很短也需要更多判断。选择高档位前先检查范围、接口和上下文；只有剩余推理难点或 medium 能力不足的证据才支持升级，并在派工说明中写明难点。架构标签和较后的修复轮次不自动触发升档。局部修复与限定复审只有在原因、行为、方案、影响和验证清楚，且无需设计或跨任务判断时，才可用较轻档位。你的预算仍然有效。

精确型号名单与角色权限放在[派工模板](references/worker-policy-template.md)，由它生成项目的 `worker-policy.md`。优化的是正确交付的总成本：规划、上下文交接、实施、审查和返工。原子边界应让明确工作适合较轻档位，让剩余判断落在默认角色能处理的范围内。大量任务需要高档位时回看计划，不设模型使用比例配额。失败后先分类原因，再判断是否升级；型号不可用时明确报告，不静默换档，也不回退旧型号。在普通能力上限仍无法收敛、出现更早的停滞证据或达到五轮上限时，进入执行恢复。

### 仅 Claude Code

**怎么挑帮手。** Claude 的帮手全部使用 Opus，并按角色分四档：light 做满足同一低风险条件的 mechanical 或 standard 小任务；standard 是常规实施和初审的默认档；deep 用于写明了具体推理难点的任务；只读的 diagnosis 用于独立诊断和复杂计划的整体终审。Claude Code 只能通过 agent 定义文件设置帮手的 effort，所以在 Claude Code workspace 里，`nhk-bootstrap` 会按 [agent 定义模板](references/claude-agents-template.md) 提议在 `.claude/agents/` 下添加四个可选文件。我们建议接受；如果拒绝，帮手仍然使用 Opus，effort 跟随你的会话设置。新建的 `.claude/agents/` 目录要开一个新会话，Claude Code 才能发现。Fable 留在主线程，而且要由你选择或同意。
...
**审查。** SDD 为每个原子任务或符合条件的机械批次安排一位独立只读审查者，分别给出需求符合度与实现质量结论，两项都要通过。初审默认使用中间档位，只有审查本身确实困难才用高档位。复杂计划的整体终审使用高档位，需要更深推理时可用 max；除此之外，max 只用于独立诊断。内部步骤不单独派审查者。审查依据固定版本、约束、实际 diff 与测试证据。只有单任务、非复杂计划，已通过的 SDD 任务审查在相同最终范围、版本和证据上覆盖全部要求与改动时，才能兼作终审；任何变化都要重新判断。多任务和复杂计划保留整体终审。Native 执行保留自己的逐任务验证和一次独立整体终审，不增加逐任务子代理审查。合并审查不增加修复或恢复次数。
...
**更新旧项目。** Upkeep 将 NHK 自有的完整模块默认规则改为原子任务契约，保留项目事实和已批准例外。旧 `module-implementation` / `initial-module-review` 记录只有仍指向同一个任务、相同范围和已确认批准依据时，才改为 `task-implementation` / `initial-task-review`。旧模块拆成多个任务或对应关系不明确时，报告给人决定，不把授权自动复制到新任务。仅更新安装包不会自动改写项目文件。
