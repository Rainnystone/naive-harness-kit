...
**Expected:** Separate the independent changes. The clear, low-risk small feature may use Luna max even with `Worker class: standard`; local design or integration defaults to Astra medium. Supply missing authoritative context rather than escalating. A separate investigation is justified only by a verifiable conclusion, contract, or reusable decision, defaults to medium, and allows later implementation to choose Luna or medium afresh. Batch identical transformations only with explicit files and shared verification; do not dispatch internal test steps. Preserve coupled logic and its tests. A remaining concrete reasoning difficulty may justify xhigh directly. Architecture labels, short code, and fix rounds 4–5 do not decide capability. Recheck boundaries when many tasks need higher capability, without quotas or mandatory full implementation code in the plan. Controlled planning and dispatch exercises assess rule application; static checks do not demonstrate cost savings.
...
**Expected:** Ordinary fixes prefer the original implementer; scoped re-review prefers the original independent reviewer. The two mapping fixes may use Band 1 only when cause, intended behavior, approach, impact, and verification are clear without design/cross-task judgment. The ownership decision stays with the original task owner or Band 2/3 under the difficulty rules. Small line count and reviewer origin do not make it mechanical. Band 1 permission never requires a new worker. A cheaper fresh worker needs a worthwhile self-contained handoff; batch the suitable findings. Appending judgment-bearing mechanical permission fails validation.
