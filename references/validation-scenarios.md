...
**Expected:** An atomic task is the smallest independently testable delivery with a complete verification loop worth its own review. Keep `Task N`, `Delivers`, `Blocked by`, `Worker class`, Files, Interfaces, behavior, boundary examples, concrete internal steps, verification commands, expected results, and necessary code examples. Two independently acceptable deliveries within one export feature may be separate tasks. Each keeps its required implementation, tests, configuration, migration, and docs; a coupled transaction, permission decision, or recovery path stays intact. File count, code size, and elapsed time are clues, not thresholds. Internal steps stay within the task. Wide migrations preserve expand → migrate batches → contract and explicit integration when batches cannot stay green independently. NHK sizing, planning detail, routing, reuse, and timing override conflicting generic workflow defaults without replacing Superpowers test or review prompts.

### C9a. Sizing and capability reinforce each other

**Setup:** A plan contains two independently acceptable changes within one feature, a small feature with an established approach but no complete code listing, an integration decision, a missing interface, twelve identical mechanical edits, and a tightly coupled concurrency problem.

**Expected:** Separate the independent changes. The clear, low-risk small feature may use `light` even with `Worker class: standard`; local design or integration defaults to `standard`. Supply missing authoritative context rather than escalating. A separate investigation is justified only by a verifiable conclusion, contract, or reusable decision, defaults to `standard`, and allows later implementation to choose `light` or `standard` afresh. Batch identical transformations only with explicit files and shared verification; do not dispatch internal test steps. Preserve coupled logic and its tests. A remaining concrete reasoning difficulty may justify `deep` directly. Architecture labels, short code, and fix rounds 4–5 do not decide capability. Recheck boundaries when many tasks need higher capability, without quotas or mandatory full implementation code in the plan. Controlled planning and dispatch exercises assess rule application; static checks do not demonstrate cost savings.

### C10. Superpowers task-brief compatibility

**Setup:** A temporary Superpowers plan has binding global constraints outside its Task section. The task includes the three NHK fields, Files, Interfaces, steps, commands, expected results, and necessary code examples.

**Expected:** The current Superpowers `task-brief` extracts the complete task section, including the added fields and ordinary execution detail. Because it omits plan-level constraints, the dispatcher copies those constraints into the task or attaches a self-contained handoff. The worker receives every binding constraint and relevant interface through upstream brief, report, diff package, and ledger artifacts without main-thread history. NHK adds no plugin dependency or alternate extractor.
...
### D3. Capability tiers and availability

**Setup:** A packet is clear low-risk work, an ordinary bounded implementation, or architecture/high-uncertainty work. In one run, the selected tier's configuration is unavailable.

**Expected:** Capability Tiers defines `light`, `standard`, `deep`, and `audit` once, without platform names. The Codex catalog maps them exactly to GPT-6 Luna max, GPT-6 Sol xhigh, GPT-6 Astra medium, and GPT-6 Astra xhigh; GPT-6 Astra max is not a worker preset. Ordinary local design or integration defaults to `standard`; a small, clear, low-risk standard feature and an independent deterministic conversion may use `light` without complete implementation code in the plan. Before `deep`, check sizing, interfaces, and context. A concrete remaining reasoning difficulty permits direct `deep` dispatch with a one-sentence reason; no lower-tier failure is required. Failure classification distinguishes scope/context/environment/verification from capability; only demonstrated capability limits justify escalation by one tier. The ordinary implementation ceiling is `deep`, with earlier stagnation and five-round recovery bounds intact. Unavailability does not authorize another tier, older model, or the `audit` configuration as fallback. Initial reviews independently default to `standard`. Main-thread model/effort does not constrain these permissions. Legacy Band routes, a duplicated, missing, or unrecognized tier row, and presets outside the catalog fail source/final validation; tier words in ordinary project facts, outside routing syntax, stay valid. Static checks validate declared rules, not actual task difficulty or cost savings.

### D4. Review gates and the audit tier
...
**Expected:** All initial reviews default to `standard`, including standalone mechanical work; `deep` requires difficulty in the review itself. `light` may perform low-risk scoped re-review, never initial review. Implementer configuration does not automatically determine reviewer configuration. Use the applicable upstream task-reviewer, re-review, or final-review prompt and verify claims against fixed revisions, constraints, diff, and test evidence.
...
**Setup:** An export task review finds two deterministic mapping mistakes and one unresolved ownership decision.

**Expected:** Ordinary fixes prefer the original implementer; scoped re-review prefers the original independent reviewer. The two mapping fixes may use `light` only when cause, intended behavior, approach, impact, and verification are clear without design/cross-task judgment. The ownership decision stays with the original task owner or `standard`/`deep` under the difficulty rules. Small line count and reviewer origin do not make it mechanical. `light` permission never requires a new worker. A cheaper fresh worker needs a worthwhile self-contained handoff; batch the suitable findings. Appending judgment-bearing mechanical permission fails validation.

### D7c. Review consolidation preserves scope and recovery bounds

**Setup:** One non-complex task passes specification and quality review at fixed BASE/HEAD with complete evidence. Compare a multi-task plan, a complex plan, changed HEAD, changed scope, new evidence, and a user-selected native execution run.

**Expected:** Only the single-task non-complex SDD plan with all requirements, changes, evidence, and identical final scope/version can reuse its passed task review as final review. The other cases retain whole-change review or require re-evaluation; stale approval cannot close them. SDD gives each task or qualifying batch one initial independent read-only reviewer, with no internal-step reviewers. Native execution retains its own task verification and one independent whole-change final review, without per-task subagent reviews. Scoped re-review addresses original findings and fix regressions; resolve cannot-verify before completion. Consolidation never resets or extends task/gap counts, recovery, or final fix-wave bounds. Splitting tasks, changing a model, or revising a plan preserves the same acceptance-gap count. Additional conflicting consolidation declarations fail source/final validation.
...
**Setup:** In a Claude Code workspace with NHK agent definitions, dispatch a clear mechanical change, a small low-risk standard feature, an ordinary integration task, a task with a stated reasoning difficulty, an independent diagnosis, and a built-in agent after the main thread selected Fable.

**Expected:** Every Claude worker runs Opus. The mechanical change, eligible small feature, integration task, and every initial review use `nhk-standard`, which serves both `light` and `standard`; the stated difficulty and a non-complex whole-change final review use `nhk-deep`; the diagnosis and a complex whole-change final review use read-only `nhk-audit`. Dispatch through a definition omits the per-invocation model. `worker-policy.md` maps tiers to definition names only, while the definitions carry model and effort; the top effort level stays with the human-chosen main thread. Fable is permitted only when the human explicitly chooses or approves it for the main thread, and the built-in agent receives `model: opus`, so no worker inherits Fable.
...
**Expected:** Bootstrap offers the definitions from `claude-agents-template.md`, recommends them, and creates the three files only after agreement, telling the human that a new `.claude/agents/` directory needs a fresh session. A decline leaves the seven-surface foundation complete; workers then receive `model: opus` and run at session effort. The Codex-only workspace is never offered Claude definitions. Source validation checks `nhk-standard`, `nhk-deep`, and `nhk-audit` in order, `model: opus`, strictly rising worker effort below the top level, policy-pointing descriptions without proactive delegation, write denial for `nhk-audit`, and the retired-definition contract; it does not verify the runtime effort a live worker receives.
...
**Setup:** The installed bundle has the GPT-6 tier rules, while a complete workspace retains the former three-band policy, GPT-6 Astra max diagnosis, and an exception record whose preset is GPT-6 Astra xhigh for task implementation. Project paths and document structure are still correct. One project fact and one explicitly authorized exception must remain.

**Expected:** An update check or explicit upkeep request routes to upkeep; missing foundation files still route to bootstrap first. Every upkeep compares the canonical instruction, all five companions, and archive index against current installed references, not only visible structural drift. It repairs NHK-owned rules into the tier contract, preserves project facts and approved exceptions whose preset is still a permitted `light`, `standard`, or `deep` preset, and reports the Astra xhigh implementation record as an unresolved conflict for a human decision. Previously read references can be reused only when unchanged. It does not fetch updates, rewrite whole documents, or introduce version tracking. Completion accounts for every compared surface as aligned, explicitly excepted, or unresolved.

### E4a. Agent definitions follow the installed template

**Setup:** A complete Claude Code workspace has the former `nhk-light`, `nhk-standard`, `nhk-deep`, and `nhk-diagnosis` definitions, whose effort and shared body predate the installed `claude-agents-template.md`; a second workspace has none.

**Expected:** Upkeep reads the agent-definition template only for the first workspace, reconciles `nhk-standard` and `nhk-deep`, creates the missing `nhk-audit`, and preserves unrelated agent files. It never deletes or renames `nhk-light` or `nhk-diagnosis`; it reports both as retired for the human to delete. The second workspace stays without definitions; upkeep neither offers nor creates them.

### E5. Module policy migration preserves active work

**Setup:** A complete synthetic workspace retains v0.3.0 whole-module defaults, completed and in-flight Task identifiers, and unstarted work containing independent deliveries. One old module-role exception maps to the same single task and scope; another module splits into several tasks.
...
**Expected:** English and Chinese READMEs both describe five recurring jobs, four skills plus eleven controlled references, seven mandatory foundation surfaces, the sibling install layout, optional validator and its two new final-document kinds, session refresh/discovery check, the Superpowers overlay, role-authorized worker routing, dual review verdicts, bounded recovery, routing-table-as-shallow-map policy, and Claude's on-demand companion loading. They describe atomic delivery boundaries, four capability tiers shared by both platforms, sizing before capability, eligible small standard features on `light` without complete code listings, `standard` as the default implementation tier, SDD versus native review boundaries, bounded `deep` selection as the implementation ceiling, and the read-only `audit` tier for independent diagnosis and complex final review. GPT-6 Sol xhigh main-thread guidance is a human suggestion in the READMEs only; users choose model and effort. The READMEs likewise suggest Opus high for a Claude Code main thread, describe every Claude helper running Opus through three definitions with `nhk-standard` serving both `light` and `standard`, and present the agent definitions as optional, recommended, and effective only after a fresh session for a new `.claude/agents/` directory. They link the companion templates for the exact worker catalog and recovery procedure instead of carrying duplicate copies. Neither README presents scripts or tests as runtime dependencies.

**Negative check:** Source validation fails if either README loses the atomic boundary or low-risk implementation guidance, or if either or both READMEs lose the Sol main-thread recommendation, its suggestion-only status, the user's choice of model and effort, or worker permissions remaining independent of that choice. The diagnostic names each affected README.
