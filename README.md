...
NHK uses three practical Codex bands with permissions tied to the role. Complete modules default to the medium configuration; independent, deterministic, clearly specified low-risk mechanical work may use the lighter band. Before choosing the harder band, check scope, interfaces, and context. Use it only for a concrete reasoning difficulty that remains after those checks, or demonstrated medium capability limits, and name that difficulty in the brief. Known hard work can start there directly. Local repairs and scoped re-reviews qualify for the lighter band only when cause, behavior, approach, impact, and verification are clear without design or cross-module judgment. Your budget still binds.

The exact model list and role permissions live in the [worker policy template](references/worker-policy-template.md), which creates `worker-policy.md`. Most implementation should fit the default role through good module boundaries, without fragmenting delivery or moving all design work to the main agent. If many tasks need the harder band, revisit the plan; there is no quota for genuinely difficult work. Classify failures before escalating. Model unavailability is reported without silently changing bands or falling back to older models. Non-convergence at the ordinary ceiling, earlier stagnation, or the five-round bound leads to execution recovery.
...
After dispatch or resumption, and between unsolicited progress checks, the main agent waits at least 30 minutes. It prefers longer event waits within tool limits and higher-priority instructions, while responding immediately to completion, questions, concrete failures, and your messages. Empty waits or silence do not justify status checks, reminders, interruption, replacement, or duplicate investigation. This interval is neither a timeout nor a polling schedule, cache TTL, or runtime setting. Rule updates preserve active task identifiers and progress; they do not automatically regroup running plans or redispatch completed work.
...
If new causal evidence explains why earlier attempts failed, NHK allows one recovery fix and one independent re-review. Competing explanations can receive at most one fresh, read-only diagnosis using the harder configuration or max as needed. It separates observations from old hypotheses and returns evidence or a discriminating experiment; fixes and re-reviews use their own role permissions. Five rounds are a ceiling: the three-fix architecture check and earlier evidence of stagnation still apply. If evidence remains inconclusive, or recovery fails, the next decision is yours. The [recovery template](references/execution-recovery-template.md) defines the evidence and stopping rules.
