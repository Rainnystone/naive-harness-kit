...
NHK uses two practical Codex bands with unordered members and permissions tied to the role. Whole-module implementation, internal debugging, tests, integration, and initial independent review use the medium configuration. Independent, deterministic, clearly specified low-risk mechanical work may use the lighter band; its initial review has a narrower permission. Local repairs and scoped re-reviews qualify for that band only when cause, behavior, approach, impact, and verification are clear and no design or cross-module judgment is needed. A small diff or a review finding alone is insufficient. Your budget still binds.

The exact model list and availability rules live in the [worker policy template](references/worker-policy-template.md), which creates `worker-policy.md`. Medium unavailability is reported, without silently downgrading a module or borrowing a configuration reserved for complex whole-change final review. Medium non-convergence enters execution recovery.
